...
def NewFile():
    ...
    e2 = Entry(window,show="*",font="Bauhaus93 13 bold ",width=17,relief=SUNKEN)
    e3 = Entry(window,show="*",font="Bauhaus93 13 bold ",width=17,relief=SUNKEN)
...
def login():
    ...
    t2 = Entry(window,show="*",font="Bauhaus93 13 bold ",width=17,relief=SUNKEN)
...
def fifo():
    rt=Tk()
    rt.title("MISSING OBJECT SITE")
    rt.geometry("900x600+250+50")
    rt.config(background="yellow2")
    Label(rt,text="MISSING OBJECT SITE",background="cyan",fg="brown2", font="Impact 30 bold",bd=4,height=2,
...
def missing():
    ...
    Label(master, text="OBJECT",background="yellow2",fg="brown2",
    font="Bauhaus93 14 bold ",height=2,width=17,relief=FLAT).place(x=50,y=200)
    Label(master, text="PLACE",background="yellow2",fg="brown2",
...
def missing1():
    ...
    Label(master, text="OBJECT",background="cyan",fg="brown2",font="Bauhaus93 14 bold ",
    height=2,width=17,relief=FLAT).place(x=50,y=200)
    Label(master, text="PLACE",background="cyan",fg="brown2",font="Bauhaus93 14 bold ",
    ...
    height=2,width=17,relief=FLAT).place(x=350,y=200)
...
def check1():
    if(str(e1.get())=="sayali"):
        ...
        error()
def db():
    conn = sqlite3.connect('missing1.db')
    cursor1=conn.cursor()
    ...
    cursor1.execute('''CREATE TABLE IF NOT EXISTS  MISSING
         ...
         PLACE TEXT NOT NULL,
         COLOR CHAR(50))''')
    cursor1.execute("INSERT INTO MISSING (OBJECT,PLACE,COLOR) VALUES ('"+ e4.get()+"','"+e5.get()+"','"+e6.get()+"')")
...
def db1():
   ...
   all_rows = cursor1.fetchall()
   for row in all_rows:
       if((row[0]==e9.get())and(row[2]==e11.get())):
           
               rt=Tk()
               rt.title("MISSING OBJECT SITE")
               rt.geometry("900x600+250+50")
               rt.config(background="yellow2")
               lb2=Label(rt,text="OBJECT FOUND",background="cyan",fg="brown2", font="Impact 30 bold",bd=4,
               height=2,width=30,relief=RIDGE)
               lb2.place(x=130,y=255)
               count=0
       else:
            count=count+1
   if(count!=0):
        ...
        rt.geometry("900x600+250+50")
        rt.config(background="yellow2")
        lb2=Label(rt,text="OBJECT NOT FOUND",background="cyan",fg="brown2", font="Impact 30 bold",
...
root.title("MISSING OBJECT SITE")
root.geometry("900x600+250+50")
root.config(background="yellow2")
py1=Label(root,image=im2).place(x=220,y=130)
lb2=Label(root,text="MISSING OBJECT SITE",background="cyan2",fg="brown1", font="Impact 30 bold",bd=4,
height=2,width=30,relief=RIDGE)
